...
class QwenWorker(ApiModelWorker):
    '''表明QwenWorker是一种特定类型的模型工作者，专门处理与API模型相关的任务。'''
    DEFAULT_EMBED_MODEL = "text-embedding-v1"

    def __init__(
        self,
        *,
        version: Literal["qwen-turbo", "qwen-plus"] = "qwen-turbo", # 模型的版本
        model_names: List[str] = ["qwen-api"], # 模型名称
        controller_addr: str = None, # 控制器地址
        worker_addr: str = None, # 工作者地址
        **kwargs,
    ):
        # 更新kwargs字典，将model_names、controller_addr和worker_addr加入到kwargs中。
