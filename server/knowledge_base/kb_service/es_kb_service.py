    ...
    def do_init(self):
        ...
        try:
            ...
            if self.user != "" and self.password != "":
                self.es_client_python =  Elasticsearch(hosts=[f"https://{self.IP}:{self.PORT}"],
        ...
        try:
            ...
            # 参考链接：https://api.python.langchain.com/en/latest/vectorstores/langchain_community.vectorstores.elasticsearch.ElasticsearchStore.html
            if self.user != "" and self.password != "":
                ...
                es_connection=self.es_client_python,
            )
            else:
                logger.warning("ES未配置用户名和密码")
                self.db_init = ElasticsearchStore(
