...
class SummaryAdapter:
    ...
    token_max: int # 摘要最大的token数目
    _separator: str = "\n\n" # 文本分割符，默认为两个换行符
    chain: MapReduceDocumentsChain  # 档处理流程实例
    ...
    def form_summary(cls,
        ...
        # 定义MapReduceDocumentsChain实例chain，它负责整个生成和合并摘要的流程。
        chain = MapReduceDocumentsChain(
            llm_chain=llm_chain,
            document_variable_name=document_variable_name,
    ...
    def summarize(self,
                  ...
                  docs: List[DocumentWithVSId] = [] # 文档列表
                  ) -> List[Document]: # 返回一个摘要文档列表
        '''同步地调用异步方法asummarize来生成文档摘要'''
        # 如果Python版本低于3.10，使用asyncio.get_event_loop()方法获取当前事件循环。
        if sys.version_info < (3, 10):
            loop = asyncio.get_event_loop()
        else:
            try:
                # 对于Python 3.10及以上版本，尝试使用asyncio.get_running_loop()获取当前运行的事件循环。
        ...
        # 使用获取或创建的事件循环loop，调用run_until_complete方法同步执行asummarize协程。
    ...
    async def asummarize(self,
        ...
        # 创建包含最终摘要信息的元数据_metadata，包括文件描述、中间步骤和文档ID列表。
        ...
        # 创建一个新的Document实例summary_combine_doc，它包含最终的摘要内容和元数据。
